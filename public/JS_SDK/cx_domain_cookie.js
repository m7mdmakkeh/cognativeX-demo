...
(function(exports) {
	...
	var xDomainCookie = function( iframe_path, namespace, xdomain_only, iframe_load_timeout_ms, secure_only, debug ){
		...
		var _namespace = namespace || 'xdsc',						//namespace for the shared cookie in case there are multiple instances on one page - prevents postMessage collision
			...
			_cx_domain_cookie_data = {},							//shared cookie data set by the iframe after load/ready
		...
		function _inbound_postmessage( event ){
        	...
        	_cx_domain_cookie_data = data.cookies;
			_iframe_ready = true;
		...
		function _set_cookie_in_iframe( cookie_name, cookie_value, expires_days ){
			...
			document.getElementById('cx_domain_cookie_'+_id).contentWindow.postMessage(JSON.stringify(data), iframe_path );
		}
		...
		function _set_cx_domain_cookie_value( cookie_name, cookie_value, expires_days ){
			
			//if iframe isn't ready, wait for it to be ready
			if(!_iframe_ready && !_iframe_load_error){
				return _callbacks.push(function(){
					_set_cx_domain_cookie_value( cookie_name, cookie_value, expires_days);
				});
			}
			...
			_cx_domain_cookie_data[cookie_name] = cookie_value;
		}
		...
		function _get_cx_domain_cookie_value( cookie_name, callback, expires_days ){
			
			expires_days = expires_days || _default_expires_days;

			_log("_get_cx_domain_cookie_value A", cookie_name);

			//cb function to create closure for pending user callback
			function _cb( xdomain_success, cookie_val, callback ){

				_log("_get_cx_domain_cookie_value D", xdomain_success, cookie_val);
				//re-up the cookie
				_set_cx_domain_cookie_value( cookie_name, cookie_val, expires_days );
			...
			if(!_xdomain_only){
				...
				if(_existing_local_cookie_val){
					_log("_get_cx_domain_cookie_value B", _existing_local_cookie_val);
			...
			_on_iframe_ready_or_error(function( is_err ){
				...
				_log("_get_cx_domain_cookie_value C", is_err);
				//if an error occurs loading the iframe, return appropriate response w/ callback
				if(is_err) return _cb( false, null, callback );
				
				var _current_cookie_val = cookie_name in _cx_domain_cookie_data ? _cx_domain_cookie_data[cookie_name] : null;
		...
		ifr.src = iframe_path + '/cx_domain_cookie.html#' + encodeURIComponent(JSON.stringify(data));
		...
		return {
			get: _get_cx_domain_cookie_value,
			set: _set_cx_domain_cookie_value
		};
	};
